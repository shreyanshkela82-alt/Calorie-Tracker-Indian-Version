...
// Logs a completed workout to the backend so it counts toward today's
// progress (caloriesBurned). Fails silently with a console warning so a
// network hiccup doesn't block the on-screen "workout complete" state.
async function logWorkoutToBackend(level, caloriesBurned, durationMinutes) {
    ...
    try {
        const response = await fetch(`${API_URL}/workouts`, {
            ...
            body: JSON.stringify({
                workoutName: `${level.charAt(0).toUpperCase() + level.slice(1)} Workout`,
                caloriesBurned,
        ...
        if (!data.success) {
            console.warn("Could not log workout:", data.message);
        }
    } catch (error) {
        console.warn("Could not log workout to server:", error);
    }
}
...
function completeWorkout() {
  ...
  if (!workoutLogged && currentLevel) {
    workoutLogged = true;
    const plan = workoutPlans[currentLevel];
    logWorkoutToBackend(currentLevel, caloriesToBurn, plan.totalMinutes);
  }
}
